...
def load_image_description(page_dir, image_filename):
    """從同層目錄讀取圖片描述"""
    yml_path = page_dir / f"{image_filename}.yml"
    if yml_path.exists():
...
def generate_markdown(content_blocks, output_path, page_dir):
    """
    生成 Markdown，圖片路徑使用同層目錄
    
    Args:
        content_blocks: 內容區塊列表
        output_path: 輸出檔案路徑
        page_dir: 頁面目錄路徑（圖片與 .md 同層）
    """
    lines = []
    ...
    for block in content_blocks:
        if block['type'] == 'heading':
            ...
            local_path = page_dir / image_filename

            # 從同層目錄的 .yml 讀取描述
            alt = (load_image_description(page_dir, image_filename) 
                   or block['alt'] 
                   or image_filename)
            ...
            if local_path.exists():
                # 使用相對路徑（同層目錄）
                lines.append(f"![{alt}](./{image_filename})")
            else:
...
> [!IMPORTANT]
> 圖片與 `.md` 檔案放在同層目錄，不使用 `images/` 子目錄。
...
- **圖片路徑**：使用相對路徑（同層目錄，格式：`./filename.jpg`）
- **不使用 `images/` 子目錄**
- 保持原始網頁的內容順序
- 圖片需包含 alt text（從 `.yml` 描述檔讀取）
