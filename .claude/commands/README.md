...
| Skill             | 說明           | 自動觸發時機                      |
| ----------------- | -------------- | --------------------------------- |
| `agent-browser`   | 瀏覽器自動化   | 網頁測試、表單填寫、截圖時        |
| `content-build`   | 內容建置流水線 | 建立網頁、處理圖片、內容建置時    |
| `doc-sync`        | 文件同步規則   | 修改 GUIDELINES、SOP、Commands 時 |
| `run-log`         | 執行日誌記錄   | git commit 成功後自動觸發         |
| `sop-consistency` | SOP 一致性檢查 | 執行結構變更、目錄重構時          |
| `web-crawler`     | 網站爬蟲工具   | 爬取網站、備份網站、crawl 時      |
...
| 指令            | 說明                              | 觸發時機 |
| --------------- | --------------------------------- | -------- |
| `/check_assets` | 資源檢查（圖片描述檔、MD/YML 配對） | 每日     |
...
| -------------------- | ----------------------------------------------- |
| `/update_doc`        | -                                               |
| `/check_docs`        | `05_agent_refactor.md`                          |
| `/check_assets`      | `05_agent_refactor.md`                          |
